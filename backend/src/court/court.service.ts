...
export class CourtService {
  ...
  async getCourts() {
    return this.courtModel.find().populate({
      path: 'currentMatchId',
      populate: [
  ...
  async getQueuedMatches() {
    return this.matchModel.find({ status: 'Queue' })
  ...
  async assignMatchToCourt(matchId: string, courtId: string) {
    ...
    if (court.status === 'In Use') throw new BadRequestException('Court is already in use');

    const match = await this.matchModel.findById(matchId);
    if (!match) throw new NotFoundException('Match not found');
    if (match.status !== 'Queue') throw new BadRequestException('Match is not in Queue status');

    court.status = 'In Use';
    court.currentMatchId = match._id as any;
    await court.save();

    match.status = 'Live';
    match.courtId = court._id as any;
    await match.save();
